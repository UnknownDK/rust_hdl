    ...
    for &(name, input, minimum_standard) in CORPUS {
        for standard in [
            ...
            for keyword_case in [KeywordCase::Lower, KeywordCase::Upper] {
                ...
                .unwrap_or_else(|error| panic!("{name} {standard:?} {keyword_case:?}: {error:?}"));
                ...
                .unwrap();
                assert_eq!(output, twice, "{name} {standard:?} {keyword_case:?}");
                eprintln!(
                    "{name} {standard:?} {keyword_case:?}: preservation and idempotency passed"
                );
            }
        }
    }
